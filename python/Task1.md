...
​	**input函数：**返回的类型是string类型；同时，输入是可以带提示语的。
...
​		***注意：在python3中raw_input()和input()进行整合，去除了raw_input()。其接收任意任性输入，将所有输入默认为字符串处理，并返回字符串类型。***
...
1）input 得到的数据到底是什么类型
2）多个参数接收input的输入，是怎么解包的
3）如果输入的个数不确定，怎么处理
4）要判断输入的是不是float 类型
5）要对输入的参数格式输出，规定参数只能是str或者int 或者float类型
```

​		**1）input得到的数据到底是什么类型**

​		在python 3 里，一切input的得到的东西，都是str类型。
...
​		![1557885564701](assets/1557885564701.png)

​		这种情况就是不清楚input得到的东西是什么形式，在python3中，input返回的都是str类型的。

​		也就是说，输入1，2，4其实是返回的字符串‘1，2，4’。
...
​		前面是三个变量，而后面是一个字符串，那么后面自然就要解包，吧字符串解包成单个字符。

​		但是‘1，2，4’解包之后是‘1’  ’，‘  ’2‘  ’，‘   ’4‘  五个字符，所以会报错。

​		![1557886476261](assets/1557886476261.png)

​		**解决方法：**

​		No.1	字符之间不用逗号或者空格作间隔符

​		![1557886645375](assets/1557886645375.png)

​		No.2	在input后面添加.split（），输入时使用空格分隔

​		![1557886730604](assets/1557886730604.png)

​		其实就是对string’1 2 4‘ 的split分割。
...
​		![1557887209235](assets/1557887209235.png)

​		

​		**4）要判断输入的是不是float 类型**

​		当输入的是float的时候，把string转换成float是不会报错的，所以如果报错，那么这个输入就不是floa类型。

​		![1557887574150](assets/1557887574150.png)

​		![1557887595093](assets/1557887595093.png)



​		**5）如果要对输入的参数格式化输出，规定参数只能是str或int或float类型**
